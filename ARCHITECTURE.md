...
| `packages/ts/memorax-code-codebuddy-adapter` | CodeBuddy/WorkBuddy plugin manifest, UserPromptSubmit/Stop Hooks, native JSONL transcript bridge, memory retrieval/writeback, trace/reconciliation, Skill materialization, and supervised Repo Memory maintenance | CodeBuddy transcript interpretation inside the Backend or model-provider configuration | `.codebuddy-plugin`, `hooks`, `src`, and `skills/memorax-code` |
...
sequenceDiagram
  ...
  Provider-->>Runtime: initial Add response
  Runtime->>Obs: emit the Add result
  Obs->>Trace: persist the writeback event
```
...
provider and memory code independent from the concrete trace Store and Backend
debug logger.
