...
test("Backend runs DSH Search, normalized Trace, and Add from one native Turn interval", async () => {
  ...
  const fetchImpl = async (requestUrl, init) => {
    ...
    if (request.url.endsWith("/v1/memories/search")) {
      return new Response(JSON.stringify({
        success: true,
        data: {
          task_id: "dsh-memory-search",
          status: "completed",
          data: [{
            id: "memory-dsh",
            memory: "Shared DSH memory.",
            score: 0.95,
    ...
    return new Response(JSON.stringify({
      success: true,
      data: { task_id: "dsh-memory-add", status: "queued" },
    }), {
      status: 200,
      headers: { "content-type": "application/json" },
    });
  ...
  const turnStart = {
    ...
    cwd: interval.cwd,
    prompt: "Find the relevant DSH integration guidance.",
  };
  try {
    ...
    await waitFor(
      ...
      "DSH writeback did not call MemoraX add",
    );
    const addRequest = requests.find((request) => request.url.endsWith("/v1/memories/add"));
    assert.deepEqual(addRequest.body.messages.map(({ role, content }) => ({ role, content })), [
      { role: "user", content: "Implement the DSH adapter." },
      { role: "assistant", content: "I will inspect.\n\nThe adapter is ready." },
    ]);
    assert.equal(JSON.stringify(addRequest.body).includes("recalled memory"), false);
    assert.equal(JSON.stringify(addRequest.body).includes("private tool result"), false);
