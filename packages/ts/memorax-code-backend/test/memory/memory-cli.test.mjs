...
for (const clientCase of [
  {
    label: "Claude",
    client: "claude",
    turnId: "claude-prompt",
    tracePaths: claudeTracePaths,
    writeCurrentTurn(root, sessionId, workspace, turnId) {
      return writeCurrentClaudeTurn(traceContextFromClaudeHookBody({
  ...
  },
  {
    ...
    tracePaths: openCodeTracePaths,
    writeCurrentTurn(root, sessionId, workspace, turnId) {
      return writeCurrentTraceTurn(traceContextFromOpenCodeHookBody({
        sessionId,
  ...
  test(`memory CLI search binds to ${clientCase.label} trace instead of the same-id Codex trace`, async () => {
    const root = await mkdtemp(join(tmpdir(), `memorax-code-cli-${clientCase.client}-trace-`));
    const sessionId = `shared-${clientCase.client}-cli-session`;
    const codexWorkspace = join(root, "codex-workspace");
    const clientWorkspace = join(root, `${clientCase.client}-workspace`);
    const nestedCwd = join(clientWorkspace, "src");
    await Promise.all([
    ...
    await clientCase.writeCurrentTurn(root, sessionId, clientWorkspace, clientCase.turnId);
    const requests = [];

    const result = await runMemoryCli(["search", "--query", `${clientCase.label} trace bridge`], {
      env: {
        CODEX_THREAD_ID: sessionId,
        MEMORAX_CODE_MEMORY_CLI_TRACE_CLIENT: clientCase.client,
    ...
    assert.equal(result.ok, true);
    assert.equal(requests[0].user_id, `user-1@${clientCase.client}-workspace`);
    const events = (await readFile(clientCase.tracePaths(root).eventsJsonl(sessionId), "utf8"))
      .trim()
      .split("\n")
      .map((line) => JSON.parse(line));
    assert.equal(events.length, 1);
    assert.equal(events[0].type, "memory_cli_search");
    assert.equal(events[0].trace.client, clientCase.client);
    assert.equal(events[0].trace.turn_id, clientCase.turnId);
    assert.equal(events[0].trace.context_origin, "current-turn-file");
