...
export function runBackendCli(argv = process.argv): void {
  ...
  if (argv.includes("--help") || argv.includes("-h")) {
    ...
    console.log([
      ...
      "Memory CLI: memorax-cli status, memorax-cli search --query TEXT [--sources dialogue|document|dialogue,document] [--document-id ID]... [--session-id ID], memorax-cli add --memory TEXT --type TYPE --reason REASON [--session-id ID] [--content-type code|dialogue] [--mode default|raw|pre_summarized]",
    ...
    return [
      ...(npmPackageRemoved
        ...
        : clientName
          ...
          : []),
      ...(clientName
        ? [green(`Restart or refresh ${clientName} so it drops the removed ${removalNoun}.`)]
        : []),
    ];
  }
  return [];
...
function statusGuidance(report: MemoraxCodeStatusReport): string[] {
  ...
  if (report.ok) {
    ...
    if (report.dshAdapter && adapters.length === 1) {
      ...
      return [
        green("MemoraX Code is ready for new DSH sessions."),
        "Restart or refresh DSH only if its Profile integration changed or MemoraX Code is not active on the next prompt.",
      ];
    }
    return [
