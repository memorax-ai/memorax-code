...
export const TRACE_CLIENTS: readonly TraceClient[] = ["codex", "claude", "dsh", "opencode", "kimi"];
...
export type KimiTraceConfig = ClientTraceConfig;
...
export type KimiTracePaths = ClientTracePaths;
...
export const KIMI_TRACE_DEFAULT_CONFIG: KimiTraceConfig = {
  enabled: true,
  captureContent: true,
  retentionDays: 7,
  maxEventChars: 20_000,
...
const TRACE_DEFAULT_CONFIGS: Readonly<Record<TraceClient, ClientTraceConfig>> = {
  ...
  kimi: KIMI_TRACE_DEFAULT_CONFIG,
};
...
export function kimiTraceConfigFromEnv(
  env: Record<string, string | undefined> = process.env,
  fileConfig?: MemoraxCodeConfig,
): KimiTraceConfig {
  return clientTraceConfigFromEnv("kimi", env, fileConfig);
}
...
export function kimiTracePaths(memoraxCodeHome = memoraxCodeHomeForTrace(process.env)): KimiTracePaths {
  return clientTracePaths("kimi", memoraxCodeHome);
}
