...
export function runNpmPreinstallPackageTransition(options = {}) {
  ...
  if (!existsSync(pidPath)) return { disposition: "noop" };
  ...
  withJsonFileLock(transitionPath, () => {
    ...
    if (pidState.status === "absent") return;
    shouldStop = true;
    if (pidState.status !== "alive") return;
    transition = {
      version: PACKAGE_TRANSITION_RECORD_VERSION,
      state: "retiring",
...
export async function runNpmPostinstallPackageTransition(options = {}) {
  ...
  return await withJsonFileLockAsync(transitionPath, async () => {
    ...
    await withSetupCompletionLock(memoraxCodeHome, async (completion) => {
      if (completion.status !== "absent") return;
      const write = writeSetupCompletionRecord({
        memoraxCodeHome,
        completedAt: nowIso(options),
        completedByVersion: packageVersion,
      });
      if (write.durability !== "confirmed") {
        throw transitionError("SETUP_COMPLETION_DURABILITY_UNCERTAIN", "durable persistence of setup completion could not be confirmed");
      }
    });
