...
const completionRelativePath = join("runtime", "setup", "setup-completion.json");
...
test("a live Backend is retired before replacement and restored once afterward", async () => {
  ...
  try {
    ...
    assert.equal(completion.completedByVersion, "9.8.7-test");
...
test("invalid or unsupported setup completion fails closed after verification", async (t) => {
  for (const [name, completionText] of [
    ["invalid", "{not-json\n"],
    ["unsupported", `${JSON.stringify(validCompletion({ version: 2 }), null, 2)}\n`],
  ]) {
    await t.test(name, async () => {
...
test("a valid setup completion record is preserved", async () => {
  const completion = validCompletion({ completedByVersion: "1.2.3-existing" });
...
test("transition and setup records use private POSIX permissions", {
  ...
  try {
    ...
    assert.equal((await runEntry(fixture, "postinstall")).code, 0);
    assert.equal((await stat(fixture.completionPath)).mode & 0o777, 0o600);
    assert.equal((await stat(dirname(fixture.completionPath))).mode & 0o777, 0o700);
  } finally {
    await fixture.cleanup();
  }
});
...
async function createFixture({
  ...
  const completionPath = join(home, completionRelativePath);
...
appendFileSync(${JSON.stringify(logPath)}, JSON.stringify({ command, args, transitionState }) + "\\n");
...
function validCompletion(overrides = {}) {
  return {
    ...
    completedByVersion: "1.2.3-existing",
    ...overrides,
  };
}
