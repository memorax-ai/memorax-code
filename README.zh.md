...
开始前，请确保已安装 Node.js 24 或更高版本，以及 Codex、Claude Code 或 OpenCode Desktop
...
在 OpenCode 中，直接让 Agent 使用名为 `memorax-code` 的 Skill；OpenCode 会自动发现已安装的
Skill 目录。下面的指令使用产品名称，三个客户端均可直接理解。
...
周期性的 Skill 提醒、User Profile 和 Procedure Memory 指引，以及受监督的 Repo Memory 后台维护。
执行 repo-read 时会按策略维护 Repo Memory，首个符合条件的 Prompt 也会在后台初始化缺失的
bundle。可运行 `memorax-code-opencode doctor` 检查受管集成、插件运行证据和 Backend 健康状态。
...
| **Repo Memory 后台整理** | 读取 Repo Memory 时，在后台维护仓库结构、代码入口和历史证据；Codex、Claude Code 和 OpenCode 都会在首个符合条件的 Prompt 自动初始化缺失的 bundle。 |
