...
MemoraX Code 让 Codex、Claude Code、DeepSeek Harness、OpenCode 和 Hermes 共享一套能够持续积累的记忆。
...
开始前，请确保已安装 Node.js 24 或更高版本，以及 Codex、Claude Code、DeepSeek Harness、OpenCode 或 Hermes
...
Claude Code、DeepSeek Harness、OpenCode 和 Hermes，并为检测到的 Coding Agent 启用集成。按照终端提示输入
...
克隆示例仓库，并在项目目录中打开 Codex、Claude Code、DeepSeek Harness、OpenCode 或 Hermes：
...
调用该 Skill。在 OpenCode 或 Hermes 中，直接让 Agent 使用名为 `memorax-code` 的 Skill。下面的指令使用产品名称，
所有受支持的客户端均可直接理解。
...
> Claude Code、DeepSeek Harness、OpenCode 和 Hermes，查看不含正文的本地活动与状态。
...
| **客户端集成** | 与 Codex、Claude Code、DeepSeek Harness、OpenCode 和 Hermes 集成，触发记忆检索、提醒和写入。 |
...
Claude Code、DeepSeek Harness、OpenCode 和 Hermes。
